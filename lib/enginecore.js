...
function streamGCodeFile (filename) {
	...
	gcodedata.filepath = process.cwd()+'/gcode/'+filename;
	console.log('Streaming GCODE file: '+gcodedata.filepath);

	// send event to trigger getGCodeFileLinesCount(..) function
	evnt.getEventEmitter().getGCodeFileLinesCount(gcodedata);
...
function spRxCallback (data) {
	...
	if (data.indexOf("ok") != -1) {
		// send event to trigger sendGCodeBlockData(..) function
		evnt.getEventEmitter().sendGCodeBlockData(gcodedata);
	}
};
